...
const signupUser = async (formData: SignupFormData) => {
  const response = await fetch(
    "https://liveserver.nowdigitaleasy.com:5000/client/signup",
    {
      method: "POST",
      headers: {
                    ...
                    <div key={index} className="flex mr-4 flex-col">
                      ...
                      {item.field === "phone_number" ? (
                        ...
                        <>
                          <label
                            htmlFor={item.field}
                            className="ml-2 font-source-sans-pro text-start text-lg font-400 font-roboto text-[13px] text-[#313131]"
                          >
                            {item.label}
                          </label>
                          <input
                            type={item.type}
                            {...register(item.field as keyof SignupFormData, {
                              required: true,
                            })}
